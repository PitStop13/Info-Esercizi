...
namespace Scrutinio
{
    public partial class FormScrutinio : Form
    {
        ...
        /// <summary>
        ///     Visualizza gli studenti con i voti della materia selezionata compresa tra 2 valori
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void btnVotoTraDue_Click(object sender, EventArgs e)
        {
            string materiaSelezionata = cmbMateria.SelectedItem.ToString();
            int posMat = cmbMateria.SelectedIndex;
            int votoMin, votoMax;
            string stud = "";

            while (!int.TryParse(Interaction.InputBox("Min:"), out votoMin) || votoMin < 4 || votoMin > 10) ;
            while (!int.TryParse(Interaction.InputBox("Max:"), out votoMax) || votoMax < 4 || votoMax > 10 || votoMax < votoMin) ;

            for(int i = 0; i < ClsScrutinio.Voti.GetLength(0); i++)
            {
                if (ClsScrutinio.Voti[i, posMat] >= votoMin && ClsScrutinio.Voti[i, posMat] <= votoMax)
                {
                    stud += ClsScrutinio.Studenti[i] + " ";
                }
            }

            MessageBox.Show($"Gli studenti, della materia {materiaSelezionata}, che hanno un voto compreso tra {votoMin} e {votoMax} sono: {stud}");
