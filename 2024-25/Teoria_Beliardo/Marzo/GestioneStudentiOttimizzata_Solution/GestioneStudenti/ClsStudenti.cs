...
namespace GestioneStudenti
{
    public class ClsStudenti
    {
            ...
            for (int i = 0; i < ClsStudenti.Studenti.Length; i++)
            {
                ...
                if (mediaStud >= mediaMax)
                {
                    if (mediaStud > mediaMax)
                    {
                        mediaMax = mediaStud;
                        posLibera = 0;
                        studMediaMax[posLibera] = $"{mediaMax}";
                        posLibera++;
                    }
        ...
        public static void StudentiSenzaVoti(string[] stud, ref int pos)
        {
            for (int i = 0; i < _nStudenti; i++)
            {
                ...
                if(nVoti == 0)
                {
                    stud[pos] = $"{Studenti[i].cognome} {Studenti[i].nome}";
                    pos++;
                }
            }
        }
            ...
            for (int i = 0; i < classi.Length; i++)
            {
                ...
                if (mediaClassi[i] >= mediaMax)
                {
                    if(mediaClassi[i] > mediaMax)
                    {
                        ...
                        posClassiMediaMax = 0;
                        classiMediaMax[posClassiMediaMax] = Convert.ToString(mediaMax);
                        posClassiMediaMax++;
                    }
                ...
                if(mediaClassi[i] <= mediaMin)
                {
                    if (mediaClassi[i] < mediaMin)
                    {
                        ...
                        posClassiMediaMin = 0;
                        classiMediaMin[posClassiMediaMin] = Convert.ToString(mediaMin);
                        posClassiMediaMin++;
                    }
